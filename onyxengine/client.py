...
class Onyx:
    """
    Client for interacting with the Onyx Engine API.

    Args:
        api_key (str, optional): The API key for authenticating with the Onyx Engine.
            If not provided, the ONYX_API_KEY environment variable will be used.
    ...
    .. code-block:: python
        ...
        onyx = Onyx(api_key="your_api_key")

        # Or use ONYX_API_KEY environment variable
        onyx = Onyx()

        # Load a dataset
        dataset = onyx.load_dataset('my_dataset')
    ...
    def __init__(self, api_key: Optional[str] = None):
        if api_key is not None:
            ...
            self._api_key = os.environ.get('ONYX_API_KEY')
            if self._api_key is None:
                raise ValueError(
                    "No API key provided. Either pass api_key to Onyx() or "
                    "set the ONYX_API_KEY environment variable."
                )

    def get_object_metadata(self, name: str, version_id: str = None) -> dict:
        """
        ...
        metadata = handle_post_request(
            "/get_object_metadata",
            {"object_name": name, "object_id": version_id},
    ...
    def save_dataset(self, name: str, dataset: OnyxDataset, source_datasets: List[Dict[str, Optional[str]]] = [], time_format: Literal["datetime", "s", "ms", "us", "ns", "none"] = "s"):
        """
        ...
        for source in sources:
            if self.get_object_metadata(source.name, source.id) is None:
    ...
    def load_dataset(self, name: str, version_id: str = None) -> OnyxDataset:
        """
        ...
        metadata = self.get_object_metadata(name, version_id)
        ...
        def download_dataset():
            download_object(dataset_filename, 'dataset', version_id, api_key=self._api_key)
    ...
    def save_model(self, name: str, model: Union[MLP, RNN, Transformer], source_datasets: List[Dict[str, Optional[str]]] = []):
        """
        ...
        metadata = set_object_metadata(name, 'model', model.config.model_dump_json(), sources, api_key=self._api_key)
        upload_object(model_filename, 'model', metadata['id'], api_key=self._api_key)

        # Save local copy of metadata
        full_metadata = self.get_object_metadata(name, metadata['id'])
    ...
    def load_model(self, name: str, version_id: str = None, mode: Literal["online", "offline"] = "online") -> MLP | RNN | Transformer:
        """
        ...
        def download_model():
            download_object(model_filename, 'model', version_id, api_key=self._api_key)
            with open(os.path.join(config_path), 'w') as f:
                f.write(json.dumps(metadata, indent=2))
    ...
    def optimize_model(
        ...
        response = handle_post_request("/optimize_model", {"optimization_job": optimization_job.model_dump()}, api_key=self._api_key)
